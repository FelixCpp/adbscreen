...
//
// ADBScreen change: takes the gousb.Context as a parameter (instead of
// creating/closing its own) so a single long-lived context can be shared
// across device discovery, QT-config activation, and streaming — see
// adbscreen_shared_context.go for why that matters on macOS.
func (usbAdapter *UsbAdapter) StartReading(ctx *gousb.Context, device IosDevice, receiver UsbDataReceiver, stopSignal chan interface{}) error {
	usbDevice, err := OpenDevice(ctx, device)
	if err != nil {
		return err
	}
	// Best-effort: ask libusb to detach/reattach any conflicting kernel-side
	// driver claim (e.g. macOS's own usbmuxd/AMDS) around our interface
	// claims below. Not supported on every OS/libusb build, so ignore
	// failures here — SetAutoDetach itself will surface as a normal error
	// where it actually matters (Config()/Claim()).
	_ = usbDevice.SetAutoDetach(true)
